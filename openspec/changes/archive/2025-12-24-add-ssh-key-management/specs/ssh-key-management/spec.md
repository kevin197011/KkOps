...
The system SHALL provide the ability to manage SSH keys for WebSSH authentication, including uploading, viewing, and deleting SSH keys.

#### Scenario: Upload SSH private key
- **WHEN** a user uploads an SSH private key in the WebSSH management page
...
- **WHEN** a user views the SSH key management tab in WebSSH page
- **THEN** the system SHALL display all SSH keys belonging to the current user
- **AND** the system SHALL display key metadata: name, type, fingerprint, created date
- **AND** the system SHALL NOT display the private key content
- **AND** the system SHALL allow editing key name and deleting keys
...
### Requirement: SSH Key Selection in Terminal Connection
...
#### Scenario: Auto-select default key
...
#### Scenario: Key authentication fallback
